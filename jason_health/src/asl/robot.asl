...
+success_a_navto(Robot, NurseRoom): start(Robot, NurseRoom, Nurse, ArmRoom, Arm) <- +milestone1; !a_approach_nurse(Robot).
+!a_approach_nurse(Robot): milestone1 <- a_approach_nurse(Robot).
+success_a_approach_nurse(Robot): start(Robot, NurseRoom, Nurse, ArmRoom, Arm) & milestone1 <- -milestone1; +milestone2; .send(nurse1, tell, milestone1); .send(nurse1, tell, trigger_a_authenticate_nurse(Robot, Nurse)); !a_authenticate_nurse(Robot, Nurse).
...
+success_a_navto(Robot, ArmRoom): start(Robot, NurseRoom, Nurse, ArmRoom, Arm) & milestone6 <- -milestone6; +milestone7; .send(arm2, tell, milestone6); .send(arm2, tell, trigger_a_approach_arm(Robot, Arm)); !a_approach_arm(Robot, Arm).
...
+success_a_open_drawer(Robot): start(Robot, NurseRoom, Nurse, ArmRoom, Arm) & milestone8 <- -milestone8; +milestone9; .send(arm2, tell, milestone8); .send(arm2, tell, trigger_a_pick_up_sample(Arm, Robot)); !a_pick_up_sample(Arm, Robot).
