...
public class RosBridge {
	private static final Logger logger = Logger.getLogger(RosBridge.class.getName());
	...
	protected final Map<String, FragmentManager> fragementManagers = new HashMap<>();
	protected final Set<String> publishedTopics = new HashSet<>();
	protected final Map<String, RosBridgeSubscriber> listeners = new ConcurrentHashMap<>();
	protected final Map<String, ServiceResponseDelegate> pendingRequests = new ConcurrentHashMap<>();
	protected final Map<String, ServiceCallback> serviceCallbacks = new ConcurrentHashMap<>();
	...
	public void connect(String rosBridgeURI, boolean waitForConnection) {
		...
		try {
			...
			logger.log(Level.SEVERE, "Error ao conectar",e);
		}
		...
		synchronized(this){
			while(!this.hasConnected){
				try {
					...
					logger.log(Level.SEVERE, "Could not wait for connection", e);
				}
			}
		}
	}
	...
	public void closeConnection() {
		// TODO close all clients and unadvertise services
		...
		try {
			...
			if(node.has("op")){
				...
				switch (op) {
					...
					case "fragment" -> this.processFragment(node);
					case "service_response" -> {
						if ("service_response".equals(op)) {
								...
								ServiceClient client = serviceClients.get(service);

								if (client != null) {
									client.handleResponse(id, values, result);
								}
							}
						}
					case "call_service" -> {
							...
							ServiceCallback callback = serviceCallbacks.get(service);

							if(callback != null) {
								Object result = callback.handle(args);
								sendServiceResponse(id, service, result);
							}
						}
					default -> {
					}
			...
			logger.log(Level.SEVERE, "Could not parse ROSBridge web socket message into JSON data", e);
		}
	}
	...
	public void subscribe(SubscriptionRequestMsg request, RosListenDelegate delegate) {
		...
		try {
			...
			logger.log(Level.SEVERE, "Error in sending subscription message to Rosbridge host for topic", e);
		}
	}
	...
	public void advertise(String topic, String type) {
		...
		synchronized(this.publishedTopics) {
			boolean advertised = this.publishedTopics.contains(topic);
			...
			if (!advertised) {
				...
				try {
					...
					logger.log(Level.SEVERE, "Error in setting up advertisement to " + topic + " with message type: " + type, e);
				}
			}
		}
	}
	...
	public void unsubscribe(String topic){
		...
		try{
			...
			logger.log(Level.SEVERE, "Error in sending unsubscribe message for " + topic, e);
		}
	...
	public void unsubscribeAll(){
		List<String> curTopics = new ArrayList<>(this.listeners.keySet());
	...
	public void unadvertiseAll(){
		...
		synchronized(this.publishedTopics){
			curPublishedTopics = new ArrayList<>(this.publishedTopics);
		}
	...
	public void publish(String topic, String type, Object msg){
		...
		try {
			...
			logger.log(Level.SEVERE, "Error publishing to " + topic + " with message type: " + type, e);
		}
	}
	...
	public void sendRawMessage(String message){
		...
		try {
			...
			logger.log(Level.SEVERE, "Error sending raw message to RosBridge server: " + message, e);
		}
	}
	...
	public void formatAndSend(Object o){
		...
		try {
			...
			logger.log(Level.SEVERE, "Error parsing object into a JSON message.", e);
		}
		...
		try {
			...
			logger.log(Level.SEVERE, "Error sending message to RosBridge server: " + jsonMsgString, e);
		}
	...
	/**
	 * Subscribes to a topic with the subscription parameters specified in the provided {@link SubscriptionRequestMsg}.
	 * The {@link ServiceResponseDelegate} will be notified every time there is a publish to the specified topic.
	 * @param serviceName serrvice name.
	 * @param serviceType service type.
	 */
	public ServiceClient createClient(String serviceName, String serviceType) {
		ServiceClient client = new ServiceClient(this, serviceName, serviceType);
		serviceClients.put(serviceName, client);
		return client;
	}

	public void callService(String service, Object args, ServiceResponseDelegate delegate) {
		String id = UUID.randomUUID().toString();

		pendingRequests.put(id, delegate);

		Map<String, Object> msg = new HashMap<>();
		msg.put("op", "call_service");
	...
	public void createService(String service, String type, ServiceCallback callback) {
		serviceCallbacks.put(service, callback);

		Map<String, Object> msg = new HashMap<>();
		msg.put("op", "advertise_service");
		msg.put("service", service);
		msg.put("type", type);
	...
	public void sendServiceRequest(String service, String type, Object args, String id) {
		Map<String, Object> msg = new HashMap<>();
	...
	private void sendServiceResponse(String id, String service, Object values) {
		Map<String, Object> response =
				new HashMap<>();

		response.put("op", "service_response");
		response.put("service", service);
		response.put("values", values);
		response.put("result", true);
		response.put("id", id);

		formatAndSend(response);
	}
	...
	public static class RosBridgeSubscriber{
		protected List<RosListenDelegate> delegates = new CopyOnWriteArrayList<>();
	 ...
	 * when a message has been received from Rosbridge.
	 */
	public static class RosBridgeClient{
		protected List<ServiceResponseDelegate> delegates = new CopyOnWriteArrayList<>();
		 ...
		 * @param delegates the delegates to receive messages.
		 */
		public RosBridgeClient(ServiceResponseDelegate...delegates) {
			this.delegates.addAll(Arrays.asList(delegates));
		}
		 ...
		 * @param delegate a delegate to receive messages from Rosbridge.
		 */
		public void addDelegate(ServiceResponseDelegate delegate){
			this.delegates.add(delegate);
		}
		 ...
		 * @param delegate the delegate to stop receiving messages.
		 */
		public void removeDelegate(ServiceResponseDelegate delegate){
			this.delegates.remove(delegate);
		}
		 ...
		 */
		public void receive(JsonNode data, String stringRep){
			for(ServiceResponseDelegate delegate : delegates){
				delegate.receive(data, stringRep);
			}
		}
